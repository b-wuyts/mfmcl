...
findTreeDecomposition[gr_Graph,OptionsPattern[]]:=(
	g=IndexGraph[gr];
	Voriginal=VertexList[gr];
	idxMap=Thread[Range[Length[Voriginal]]->Voriginal];
	V=VertexList[g];
	...
	{jnodes2,jedges2}={jnodes,jedges}/.idxMap;
	deepSort/@{jnodes2,jedges2}
);
...
checkTreeDecomposition[g_,{jnodes_,jedges_}]:=(
	...
	existFactors=deepSort@Union[List@@@EdgeList[g]];
	...
	(* data structure checks, assuming connected graphs *)
	...
	If[Length[jedges]>0&&Union[fl1@jedges]!=jnodes,Print["nodes and edges don't match"];isok=False];
